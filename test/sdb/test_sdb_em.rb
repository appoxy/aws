...
#  end
  ...
  def test_03_get_attributes
    # get attributes
    values = @sdb.get_attributes(@domain, @item).get[:attributes]['Jon'].to_a.sort
...
#  end
  ...
  def test_05_replace_attributes
    ...
    values = @sdb.get_attributes(@domain, @item).get[:attributes]['Jon']
  ...
  def test_07_delete_item
    ...
    values = @sdb.get_attributes(@domain, @item).get[:attributes]['Volodya'].to_a.sort
    assert_equal ['girls', 'vodka'], values
    # delete an item
    @sdb.delete_attributes @domain, @item
    sleep 1
    # get attributes (values must be empty)
    values = @sdb.get_attributes(@domain, @item).get[:attributes]['Volodya']
    assert_nil values
  end
...
#    sleep 1
#    value = @sdb.get_attributes(@domain, @item).get[:attributes]['badname'][0]
...
#  end
  ...
  def test_17_nil_attrs
    ...
    assert_nothing_thrown do
      @sdb.put_attributes(@domain, item, {:one=>nil, :two=>nil, :three=>'chunder'}).get
    end
    sleep SDB_DELAY
    assert_nothing_thrown do
      res = @sdb.get_attributes(@domain, item).get
    end
  ...
  def test_40_delete_domain
    assert @sdb.delete_domain(@domain).get, 'delete_domain fail'
    wait SDB_DELAY, 'after domain deletion'
    # check that domain does not exist
    assert !@sdb.list_domains.get[:domains].include?(@domain)
  end
